...
class MusicBot(discord.Client):
        ...
        if server.id not in self.players:
            if not create:
                raise exceptions.CommandError(
                    'البوت غير موجود في الروم اذا أردت أن يأتي إليك عليك كتابة كلمة'
                    '%sتعال' % self.config.command_prefix)
    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            if self.config.now_playing_mentions:
                newmsg = '%s - اغنيتك **%s** ابك عانها شغاله في روم %s!' % (
                    entry.meta['author'].mention, entry.title, player.voice_client.channel.name)
            else:
                newmsg = ':heart_eyes: لبى هالذوق ياناس %s: **%s**' % (
    ...
    async def update_now_playing(self, entry=None, is_paused=False):
        ...
        if self.user.bot:
            ...
            if activeplayers > 1:
                game = discord.Game(name="Developer By FARES" % activeplayers)
                entry = None
    ...
    async def cmd_مساعده(self, command=None):
        """
        Usage:
    ...
    async def cmd_السوداء(self, message, user_mentions, option, something):
        """
        Usage:
    ...
    async def cmd_هويتي(self, author, user_mentions):
        """
        Usage:
    ...
    @owner_only
    async def cmd_رابط(self, message, server_link=None):
        """
        Usage:
        ...
        if self.user.bot:
            ...
            return Response(
                "هذا رابط البوت اذا تبي يجيك اضغط على الرابط وضيفه في قروبك وشكراَ : \n{} ".format(url),
    ...
    async def cmd_شغل(self, player, channel, author, permissions, leftover_args, song_url):
        """
        Usage:
        ...
        if 'entries' in info:
            ...
            reply_text = ":kissing_heart: أبشر الحين تشتغل ياعمي **%s** %s"
        ...
        if position == 1 and player.is_stopped:
            position = 'تامر امر ياعمري :heart:'
    ...
    async def cmd_بحث(self, player, channel, author, permissions, leftover_args):
        """
        Usage:
        ...
        search_msg = await self.send_message(channel, "الحين اجيبه لك يا عمري :heart: ")
        await self.send_typing(channel)
        ...
        def check(m):
            return (
                m.content.lower()[0] in 'نعم لا' or
                # hardcoded function name weeee
                m.content.lower().startswith('{}{}'.format(self.config.command_prefix, 'search')) or
                m.content.lower().startswith('اغلق'))

        for e in info['entries']:
            result_message = await self.safe_send_message(channel, "Result %s/%s: %s" % (
                info['entries'].index(e) + 1, len(info['entries']), e['webpage_url']))

            confirm_message = await self.safe_send_message(channel, "هذا اللي تبيه ؟ `نعم` , `لا` أو `اغلق` وشكراَ")
            ...
            # They started a new search query so lets clean up and bugger off
                    ...
                    response_message.content.lower().startswith('اغلق'):
    ...
    async def cmd_الاغنيه(self, player, channel, server, message):
        """
        Usage:
            {command_prefix}np
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                np_text = "اسم الاغنيه **%s** تم تشغليها بواسطة **%s** %s\n " % (
                    player.current_entry.title, player.current_entry.meta['author'].name, prog_str)
            else:
                np_text = "الحين تشتغل: **%s** %s\n" % (player.current_entry.title, prog_str)

            self.server_specific_data[server]['last_np_msg'] = await self.safe_send_message(channel, np_text)
            await self._manual_delete_check(message)
        else:
            return Response(
                '.مو حاط أي اغنيه الله يصلحك اذا تبي تحط اغنيه تكتب {}شغل'.format(self.config.command_prefix),
                delete_after=30
            )

    async def cmd_تعال(self, channel, author, voice_channel):
        """
        Usage:
    ...
    async def cmd_شقلب(self, channel, player):
        """
        Usage:
    ...
    async def cmd_نظف(self, player, author):
        """
        Usage:
    ...
    async def cmd_غير(self, player, channel, author, message, permissions, voice_channel):
        """
        Usage:
            {command_prefix}skip
    ...
    async def cmd_الصوت(self, message, player, new_volume=None):
        """
        Usage:
        ...
        if not new_volume:
            return Response('`%s%%` :مستوى الصوت الآن' % int(player.volume * 100), reply=True, delete_after=20)
        ...
        if 0 < new_volume <= 100:
            ...
            return Response('[ %d : %d ] مستوى الصوت حالياً' % (old_volume, new_volume), reply=True, delete_after=20)
    ...
    async def cmd_الاغاني(self, channel, player):
        """
        Usage:
    ...
    async def cmd_شيل(self, message, channel, server, author, search_range=50):
        """
        Usage:
    ...
    async def cmd_اضافه(self, channel, song_url):
        """
        Usage:
    ...
    async def cmd_خصائص(self, author, channel, server, permissions):
        """
        Usage:
    ...
    @owner_only
    async def cmd_الاسم(self, leftover_args, name):
        """
        Usage:
    ...
    @owner_only
    async def cmd_بايو(self, server, channel, leftover_args, nick):
        """
        Usage:
    ...
    @owner_only
    async def cmd_صوره(self, message, url=None):
        """
        Usage:
    ...
    async def cmd_انقلع(self, server):
        await self.disconnect_voice_client(server)
        return Response(":hear_no_evil:", delete_after=20)

    async def cmd_ريستارت(self, channel):
